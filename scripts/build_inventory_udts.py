...
# ⛔ AND A SET OF STRINGS IS NOT A SET OF FILENAMES. While signatures were staged at
# `stage/{name}.sig`, the duplicate check above was defeated by one character: `gA` and `./gA` are
# distinct strings that name the SAME file, so g2's sketch overwrote g1's, the matrix came out
# `./gA,./gA` with an off-diagonal 1.0 that is a self-comparison, and g1 was absent -- exit 0, the
# exact outcome the check exists to prevent. `../escaped` was worse: it wrote the signature OUTSIDE
# the working directory, which in Galaxy means into the job's parent. Staging by INDEX takes the
# filename out of the identifier's hands entirely; the name still travels, in `--name`, which is
# where the CSV labels come from.
